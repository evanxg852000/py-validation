...
class Validator():
    ...
    def hasRule(self, rules):
        for item in self._ruleset:
            if item['func'] in rules :
                return True
        return False
    ...
    def fails(self):
        ...
        for name, ruleset in self._rules.items():
            self._ruleset=self._parse(ruleset)
            for rule in self._ruleset:
                ...
                ok=func(name, str(value).strip(), self, *rule['args'])
                if not ok :
                    fails=True
                    if self._bail: #skip the remaining valiadators
                        break
